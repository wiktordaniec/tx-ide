...
You are not limited to the nvim companion above — you may spawn your own agent workers to parallelize independent parts of a plan (a coding worker per subsystem) or to answer an unknown before you build (a research worker). Follow **COMMON § Spawning workers** for the recipe: the `tx spawn … --cmd 'claude …'` pattern (Claude by default), the transitional both-names `--env CLAUDE_REQUIRE_WORKTREE=1 --env TX_REQUIRE_WORKTREE=1` for coding sub-workers, and the role-file priming string that makes a sub-worker load these same conventions. Always prime — a bare agent spawn gets you a worker that ignores all of this.
