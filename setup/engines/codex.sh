...
run_codex_py() {  # <install|uninstall|status>
  ...
  TX_UPDATE_LOG="$TX_HOME/codex-update/update.log" \
...
update_log = os.environ["TX_UPDATE_LOG"]
...
def status():
    ...
    print(f"  automatic update log:    {D}{update_log}{X}")
    if os.path.exists(update_log):
        with open(update_log) as handle:
            for line in deque(handle, maxlen=10):
                print(f"    {line.rstrip()}")
    else:
        print(f"    {D}not checked yet{X}")
