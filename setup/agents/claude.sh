...
#
#   install   — generate 3 C9-baked hook shims under $TX_IDE_HOME/hooks/{pre,post,end}.sh and
#               surgically REPOINT settings.json's 4 tx hook events at them (match-by-marker, so
...
# The 3 shims and the events they own. C6: Stop AND PermissionRequest both point at post.sh.
PRE_SHIM="$TX_HOME/hooks/pre.sh"     # UserPromptSubmit  → tx hook prompt-submit → WORKING
POST_SHIM="$TX_HOME/hooks/post.sh"   # Stop / Permission → tx hook stop          → WAITING (C6)
END_SHIM="$TX_HOME/hooks/end.sh"     # SessionEnd        → tx hook session-end   → IDLE (+ingest S3)
...
# Claude runs hooks with a minimal env (no shell rc). Drain the payload, then drive session state.
cat >/dev/null
...
run_settings_py() {  # <install|uninstall|status>
  ...
  TX_PRE="$PRE_SHIM" TX_POST="$POST_SHIM" TX_END="$END_SHIM" \
...
# Event → the shim command tx owns for it. PermissionRequest shares Stop's shim (C6).
EVENTS = {
...
def do_install(data):
    ...
    # Preserve the ORIGINAL pre-tx marker across idempotent re-installs (never nest a shim marker
    # inside `previous`, or uninstall would restore to a shim instead of the mailbox).
    if existing and existing.get("hook_commands") == EVENTS:
...
cmd_install() {
  ...
  header "settings.json — repoint 4 tx events (match-by-marker)"
...
cmd_status() {
  ...
  for shim in "$PRE_SHIM" "$POST_SHIM" "$END_SHIM"; do
