...
|---|---|
| `tx spawn <name> --tag TAGS [--group G] [--cwd DIR] [--cmd CMD] [--engine ENGINE] [--prompt TEXT] [--model MODEL] [--effort {1,2,3,4,5}] [--role NAME[,NAME…]] [--read-only] [--env K=V …]` | Spawn a detached tmux session. Claude/Codex agents launch from a detached `$TX_IDE_HOME/worktrees/<repository-key>/<repository>--<name>` checkout. Both footers show `<repository>--<name>` without a branch. Engine-built launches translate effort as `1=low`, `2=medium`, `3=high`, `4=xhigh`, and `5=max`; omission defaults to `3`. `--role` injects role-file contents additively into the agent's system prompt (`COMMON` always first; `user-agents/` overrides honored; Claude via `--append-system-prompt`, Codex via `-c developer_instructions=`). `--read-only` keeps shell inspection available while blocking repository edits. LLM chats are captured automatically. |
